...
# $ARGV[2] : fid espec�fico a regenerar, optativo
# ---------------------------------------------------------------
# ARCHIVOS DE ENTRADA.
# ------------------------
# Plantillas:
...
# 1.1 - 09/2016 - SCT - Se agrega paginaci�n custom, basado en paginaci�n de tagport.
        ...
        foreach my $nombase_plt (keys %NOMBASE_PLTS) {
            ...
            if ($prontus_varglb::CACHE_PURGE_TAGPORT eq 'SI') {
                if ($prontus_varglb::CACHE_PURGE_TAGPORT_MV eq 'SI') {
                    ...
                    # Si est� desactivada la opci�n de hacer purge a la vistas, solo se hace a la principal.
                    if ($mv eq '') {
                        &lib_prontus::purge_cache($k);
                    }
                }
...
sub incluir_nrosdepag {
    ...
    # Quitar comentarios de configuraci�n.
...
sub valida_param {
    ...
    if ($FORM{'tags_id'} !~ /^[0-9]+(\/[0-9]+)*$/) {
        print STDERR "\nprontus_tags_ports.cgi: Ids de tags no es v�lido. Se aborta ejecucion.\nSintaxis: $sintaxis\n";
        exit;
    }

    if ($FORM{'fid_especif'} !~ /^[\w\-\.]*$/) {
        print STDERR "\nprontus_tags_ports.cgi: Fid no es v�lido. Se aborta ejecucion.\nSintaxis: $sintaxis\n";
        exit;
    }
