...
main: {
    ...
    my @paramsKeys = &glib_cgi_04::param();

    foreach my $key (@paramsKeys) {
        if ($key =~ /^selPorts/) {
            my $value = &glib_cgi_04::param($key);
            $value =~ /(.*?)\.\w+$/; # solo el nombre del archivo, sin extension.
            $PORT_SEL_USER{$1} = 1;
        }
    }

    ($RELDIR_EDICIONES) = $prontus_varglb::DIR_CONTENIDO . $prontus_varglb::DIR_EDIC;
    ($RELDIR_CONT_EDIC) = $RELDIR_EDICIONES . "/%%ED_NOM%%";
    ($RELDIR_CONT_SECC) = $RELDIR_CONT_EDIC . $prontus_varglb::DIR_SECC;
    ($RELDIR_CONT_HPAGE) = $RELDIR_CONT_EDIC . $prontus_varglb::DIR_HPAGES;
    ($RELDIR_TEMP_EDIC) = $prontus_varglb::DIR_TEMP . $prontus_varglb::DIR_EDIC . $prontus_varglb::DIR_NROEDIC;
    ($RELDIR_TEMP_SECC) = $RELDIR_TEMP_EDIC . $prontus_varglb::DIR_SECC;
    ($RELDIR_TEMP_HPAGE) = $RELDIR_TEMP_EDIC . $prontus_varglb::DIR_HPAGES;
...
sub limpia_dir_edic {
    ...
    foreach my $entry (@entries_dir) {
        ...
        print STDERR "entry_check[$entry_check]\n";

        if (($entry !~ /^\./g) && (-f "$dir/$entry")) {
            if (!exists $PORT_SEL_USER{$entry_check}) {
                print STDERR "borrando: $dir/$entry\n";
                unlink "$dir/$entry";
            }
        }
    }
...
sub init_seccs {
    ...
    $dir_secc_edic = $prontus_varglb::DIR_SERVER . $RELDIR_CONT_SECC;

    # Path absoluto al dir. donde residen los templates de portadas del sitio web, comunes para todas las ediciones.
    $dir_secc_tpl = $prontus_varglb::DIR_SERVER . $RELDIR_TEMP_SECC;
