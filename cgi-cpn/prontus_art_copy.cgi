...
# Saca una copia fisica de un articulo y lo copia en otro
    ...
    my @mmedia = &glib_fildir_02::lee_dir("$dir_base_mm/$oldfechac$prontus_varglb::DIR_MMEDIA");
    foreach my $file (@mmedia) {
        if($file =~ /^(.*?)$oldts(\.\w+)$/) {
            my $newmmedia = $1.$newts.$2;
            &File::Copy::copy("$dir_base_mm/$oldfechac$prontus_varglb::DIR_MMEDIA/$file", "$dir_base_mm/$newfechac$prontus_varglb::DIR_MMEDIA/$newmmedia");
        }
    }
};
# --------------------------------------------------------------------------------------------------
