...
#   Retorna : handle de salida de la sentencia
    ...
    my ($base) = $_[0];
    my ($sql) = $_[1];
    my $tab_output = $base->prepare($sql);
    $tab_output->execute;
    return $tab_output;
};
...
#   Retorna: descriptor de la respuesta. (Usar solamente fetch para cargar
#       las variables con las filas siguientes.
sub ejecutar_sql_bind { # 01.2
    ...
    $sth = $dbh->prepare($sql) or  print "Content-Type: text/html \n\n".
	        "<b>Error</><br>$sql<br>" . $dbh->errstr;
    $sth->execute or print "Content-Type: text/html \n\n".
	        "<b>Error</><br>$sql<br>" . $dbh->errstr;
