...
# Se lee el titular que había antes, para no perderlo
...
sub procesar_select {
  ...
  while ($pag_aux =~ /(<select +([^>]+ *)>.+?<\/select *>)/isg) {
    ...
    if ($parte1 =~ /name *= *['"]$nom_campo['"]/is) { # ubica control con nombre correspondiente.
      ...
      # CVI - 17/01/2014 - Se robustece la expresi�n regular
...
sub add_vtxt {
  ...
  #~ Lee la plantilla del art�culo para distintas tareas
  ...
  #~ Carga los css desde la plantilla del art�culo
