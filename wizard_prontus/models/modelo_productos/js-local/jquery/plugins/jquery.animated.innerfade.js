...
(function($) {
  ...
  $.fn.animatedinnerfade = function(options) {
    ...
    this.each(function(){
      ...
      {
        if (settings.controlBoxClass != 'none') $(this).append("<div class='"+settings.controlBoxClass+" control-panel'><a class='back-button' href='#'><img src='"+settings.controlButtonsPath+"/previous.png' alt='previous' style='border: none;' /></a> <a class='pause-button' href='#'><img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' /></a> <a class='next-button' href='#'><img src='"+settings.controlButtonsPath+"/next.png' alt='next' style='border: none;' /></a></div>");
        else $(this).append("<div class='control-panel'><a class='back-button' href='#'><img src='"+settings.controlButtonsPath+"/previous.png' alt='previous' style='border: none;' /></a> <a class='pause-button' href='#'><img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' /></a> <a class='next-button' href='#'><img src='"+settings.controlButtonsPath+"/next.png' alt='next' style='border: none;' /></a></div>");
          ...
          $(this).find(".control-panel a.next-button").bind('click', function(){pauseActivated = false; clearTimeout(mytimer); $(".control-panel a.pause-button").html("<img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' />"); $.animatedinnerfade.next(elements, settings, 1, 0, mytimer, pauseActivated);return false;});
          $(this).find(".control-panel a.back-button").bind('click', function(){pauseActivated = false; clearTimeout(mytimer); $(".control-panel a.pause-button").html("<img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' />"); $.animatedinnerfade.next(elements, settings, elements.length - 1, 0, mytimer, pauseActivated);return false;});
          $(this).find(".control-panel a.pause-button").bind('click', function(){
            clearTimeout(mytimer);
            if (!pauseActivated){
              pauseActivated = true;
              $(this).html("<img src='"+settings.controlButtonsPath+"/play.png' alt='play' style='border: none;' />");
              $(elements[0]).stop().stop();
            }else {
              pauseActivated = false;
              $(this).html("<img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' />");
      ...
      if ((parseInt($(elements[current]).css('left')) == 0) || (parseInt($(elements[current]).css('left')) == vwidth))
      {
        ...
        next = 0;
          ...
          prev = elements.length - 1;
            ...
            if (settings.controlBox != 'none')
            {
              $(this).find(".control-panel a.next-button").unbind('click'); $(".control-panel a.next-button").bind('click', function(){pauseActivated = false;clearTimeout(mytimer);$(".control-panel a.pause-button").html("<img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' />"); $.animatedinnerfade.next(elements, settings, next, current, mytimer, pauseActivated);return false;});
              $(this).find(".control-panel a.back-button").unbind('click'); $(".control-panel a.back-button").bind('click', function(){pauseActivated = false; clearTimeout(mytimer);$(".control-panel a.pause-button").html("<img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' />"); $.animatedinnerfade.next(elements, settings, prev, current, mytimer, pauseActivated);return false;});
              $(this).find(".control-panel a.pause-button").unbind('click');$(".control-panel a.pause-button").bind('click', function(){
                clearTimeout(mytimer);
                if (!pauseActivated){
                  pauseActivated = true;
                  $(this).html("<img src='"+settings.controlButtonsPath+"/play.png' alt='play' style='border: none;' />"); $(elements[current]).stop().stop();
                }else{
                  pauseActivated = false;
                  $(this).html("<img src='"+settings.controlButtonsPath+"/pause.png' alt='pause' style='border: none;' />");
                  var vwidth =  - (parseInt($(elements[current]).find("img").attr("width"))-parseInt(settings.containerwidth));
