...
main: {
    ...
    # Obtiene prontus dir.
    my $ruta = $FORM{'_FILE'};         # ej /prontus_dir/site/artic/20050823/pags/20050823163215.html
...
# Obtiene contenido de la plantilla a usar para generar la pagina imprimible.
    ...
    if ($buffer_plt eq '') {
        &aborta('Error de configuraci&oacute;n: No hay plantilla para generar p&aacute;gina para imprimir.');
    }
...
# Obtiene xml del articulo a partir de la ruta del html
    ...
    if ($cont eq '') {
        &aborta('El servicio no est&aacute; disponible en este momento. [noxmldata] '. 'Intente nuevamente m&aacute;s tarde.');
    }
...
sub aborta {
    my($msg) = $_[0];
    print "Content-Type: text/html\n\n";
    print "\n<html><body><p>Ha ocurrido un error al imprimir</p></body></html>\n";
